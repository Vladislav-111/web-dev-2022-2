    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not current_user.can(action, record=user):
                flash('У вас недостаточно прав для доступа к данный странице.', 'danger')
...
def load_user(user_id):
    ...
    if db_user:
        return User(user_id=db_user[0], login=db_user[1], role_id=db_user.role_id)
    return None
...
def login():
    if request.method == 'POST':
        ...
        with mysql.connection.cursor(named_tuple=True) as cursor:
            ...
            # cursor.execute(
            #     f"SELECT * FROM users WHERE login='{login}' AND password_hash=SHA2('{password}', 256);"
            # )
            db_user = cursor.fetchone()
        if db_user:
            login_user(User(user_id=db_user[0], login=db_user[1], role_id=db_user.role_id), 
                remember=remember_me)
            flash('Вы успешно прошли процедуру аутентификаци.', 'success')
            return redirect(url_for('index'))
