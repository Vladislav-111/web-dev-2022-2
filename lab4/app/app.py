...
def GetPassErrors(password):
    ...
    if password==None:
        ...
        for passwd in password:
            if passwd.isalpha():
                if not (bool(re.search('[а-яА-Я]', passwd)) or bool(re.search('[a-zA-Z]', passwd))):
...
def GetLoginErrors(login):
    ...
    if login==None:
        ...
        for log in login:
            if not (bool(re.search('[a-zA-Z]', log)) or log.isdigit()):
        ...
        if hashlib.sha256(byte).hexdigest() == response.password_hash:
            ...
            if new_pass == repeat_pass:
                if now_pass != new_pass:
                    ...
                    flash('Пароль был успешно изменён.', 'success')
                    return redirect(url_for('index'))
                else:
            ...
            flash('Введен неверный пароль.', 'danger')
